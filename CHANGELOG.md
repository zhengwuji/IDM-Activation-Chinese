...
### èæ¬
- **æéç¯å¢èªæ£**ï¼åå¹¶ 3 æ¬¡ç¬ç« PowerShell è°ç¨ä¸ºåæ¬¡æ´ååµ·è¡ï¼ä½¿èªæ£èæ¶ç¼©ç­è¿ 3 åï¼æé«å¯å¨ååºã
- **ç½ç»è¿æ¥è¶æ¶æ§å¶**ï¼å¨ç½ç»æ¢æµæ¶å¼å¥ 1.5 ç§è¶æ¶æ§å¶ï¼é¿åå¨æ ç½æä»£çç½ç»æéæ¶é¿æ¶é´æèµ·ã
- **ä¾¿æºç/ç»¿è²çè·¯å¾æ¢æµ**ï¼æ¯æå¨èæ¬åçº§ç®å½æ¢æµ IDMan.exeãè¥æ£æµå¶ï¼å°ç´æ¥ä½ä¸³ IDM è·¯å¾ï¼æå¤§å°æ¹ä¾¿äºä¾¿æºçç¨æ·ã
- **UAC ææåè½¨ååº**ï¼å¨ PowerShell ææåéæå¤±æçæç«¯å åªç³»ç»ä¸ï¼èªå¨éè¿çæä¸´æ¶ VBScript æ·è¡ææï¼å¢å¼ºææå®¹éæ§ã
- **èªæ£ç»æä¼å**ï¼å©ç¨åçç %errorlevel% å¤æ­ä»£æ¿ && ä¸ || æ¡ä»¶åæ¯ï¼é·é¤è¯­å¥åå exit code ç¶æçª¿éã

### éæåæä¸ CI
- **è·¿æ§æ ç­¾æ£æµ**ï¼å¨ alidate.ps1 æ ¡é£å¨ä¸å¼å¥ Validate-CmdLabelsï¼éææ£æµ goto / call : è·¿æ§çæ ç­¾æ¯å¦å­å¨ï¼é²æ­¢æ¼åå¼èµ·è¯­æ³å´©æ¾ã
- **æ¬å·å¹³è¡¡éå¯¹æ£æµ**ï¼å¨ alidate.ps1 ä¸­å å¥ Validate-Parentheses æ ¡é£ï¼éæåææ¬å·çé­åç¶æï¼æé¤ CMD å¼ææææçæ¬å·è¯­æ³å¼å¸¸ã
- **ç¼ç ä¸ CI å¼å®¹**ï¼ä¸º alidate.ps1 èæ¬æ·»å å¼ºå¶ UTF-8 è§£æç¯å¢ï¼ç¡®ä¿å¨é UTF-8 è¯­è¨ç¯å¢çæ¬å°ä¸»æºä¸è¿è¡æ¶ä¸ä¼ç±äº Git åå«ä¸­ææä»¶åèå¼åè·¯å¾å¼å¸¸ã
- **èªå¨åæå»ºåå¸ï¼CI/CDï¼**ï¼æ°å»º .github/workflows/release.yml èªå¨ååå¸å¨ä½ï¼å¨ Push å°ä¸»åæ¯æ£æµå¶çæ¬å·å¸æ´æ¶èªå¨è¿è¡è¯­æ³èªæ£ãä»£ç æåãSHA256 çæå¹¶åå¸è³ GitHub Releasesï¼åæ¶èªå¨æåå¯¹åºç CHANGELOG æ´æ°æ¥å¿ã
